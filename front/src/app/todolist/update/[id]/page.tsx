...
const page = ({params} : {params: any}) => {
  ...
  useEffect(()=>{
    ...
    if(getTodoData.data) {
      setContent(getTodoData.data.content);
    ...
    <div className='w-full flex justify-center'>
        <div className='w-11/12'>
            ...
            <div className='w-full fixed flex bottom-0 left-0 justify-between border border-black'>
              <div onClick={()=>router.back()} className='w-6/12 h-14 border border-black flex items-center justify-center'>취소</div>
              <UpdateTodoBtn paramId={params.id} content={content} dateArr={selectDate} difficulty={difficulty} />
            </div>
        </div>
    </div>
  )
